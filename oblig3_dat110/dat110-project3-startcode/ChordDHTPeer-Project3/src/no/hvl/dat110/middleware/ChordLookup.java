...
public class ChordLookup {
	...
	public NodeInterface findSuccessor(BigInteger key) throws RemoteException {
		...
		// ask this node to find the successor of key "why? this is unecessary is allready done in findSuccessor in Node - class"
...
//		Alternativ:
//		if (Util.computeLogic(key, node.getNodeID().add(BigInteger.ONE), sucN.getNodeID())) {
//			return sucN;
//		} else {
...
//		good to know wut is wut
		NodeInterface succ = node.getSuccessor();
		NodeInterface stub = Util.getProcessStub(succ.getNodeName(), succ.getPort());
		NodeInterface highest = findHighestPredecessor(key);
		
		//one liner cuhhh
		return Util.computeLogic(key, node.getNodeID().add(BigInteger.ONE), stub.getNodeID()) == true ? stub : highest.findSuccessor(key);
	}
	...
	private NodeInterface findHighestPredecessor(BigInteger key) throws RemoteException {
		...
		List<NodeInterface> ent = node.getFingerTable();
		Collections.reverse(ent);
		
		for (NodeInterface finger : ent) {
			//one liner damnn
			return Util.computeLogic(finger.getNodeID(), node.getNodeID().add(BigInteger.ONE), key.subtract(BigInteger.ONE)) == true ? Util.getProcessStub(finger.getNodeName(), finger.getPort()) : (NodeInterface) node; 
		}
